...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    console.log(
      `📁 Directorio de destino: ${uploadDir} para ${file.originalname}`,
    );
    cb(null, uploadDir);
  },
...
const upload = multer({
  storage: storage,
  limits: {
    fileSize: 20 * 1024 * 1024, // 20MB límite para documentos
  },
  fileFilter: (req, file, cb) => {
    ...
    const allowedDocumentMimeTypes =
      /application\/(pdf|msword|vnd\.openxmlformats-officedocument\.wordprocessingml\.document)|text\/(plain|rtf)/;

    // Extensiones y tipos MIME peligrosos (bloqueados)
    const dangerousExtensions =
      /exe|bat|cmd|com|pif|scr|vbs|js|jar|php|asp|aspx|sh|ps1|py|rb|pl|cgi/;
    const dangerousMimeTypes =
      /application\/(x-executable|x-msdownload|x-msdos-program|x-sh|x-python|x-ruby|x-perl|x-php|vnd\.ms-cab-compressed|x-zip-compressed|x-gzip-compressed|x-tar|x-rar-compressed)/;
    ...
    if (
      dangerousExtensions.test(extname) ||
      dangerousExtensions.test(filename) ||
      dangerousMimeTypes.test(file.mimetype)
    ) {
      console.log(`🚫 ARCHIVO PELIGROSO BLOQUEADO: ${file.originalname}`);
      return cb(
        new Error(
          "Tipo de archivo no permitido por seguridad. No se permiten ejecutables, scripts, archivos comprimidos o archivos potencialmente peligrosos.",
        ),
      );
    }
    ...
    // Permitir documentos
    if (
      allowedDocumentExtensions.test(extname) &&
      allowedDocumentMimeTypes.test(file.mimetype)
    ) {
    ...
    console.log(
      `❌ Archivo no permitido: ${file.originalname} (MIME: ${file.mimetype}, Ext: ${extname})`,
    );
    cb(
      new Error(
        "Tipo de archivo no permitido. Solo se permiten: imágenes (JPEG, PNG, GIF, BMP, WebP), audio (MP3, WAV, OGG, M4A, WebM, FLAC, AAC), y documentos (PDF, Word, TXT). No se permiten ejecutables, scripts, archivos comprimidos o archivos potencialmente peligrosos.",
      ),
    );
  },
});
  ...
  return {
    ...
    sendMessage: async (data) => {
      try {
        ...
        console.log(
          `📥 Status del webhook GL: ${response.status} ${response.statusText}`,
        );
        ...
        if (response.status === 404) {
          ...
          console.log(
            "⚠️  Webhook responde 200 pero con JSON vacío/incompleto",
          );
        ...
        if (!response.ok) {
          ...
          if (errorText.includes("Unused Respond to Webhook")) {
            ...
            console.log(
              "🏗️ El workflow ya incluye: Webhook → Preparar Mensaje → Agentes IA → Respuesta\n",
            );
          }

          throw new Error(
            `Error en webhook GL: ${response.status} - ${errorText}`,
          );
        }
        ...
        try {
          ...
          if (
            ...
            throw new Error(
              "WORKFLOW RESPONDE VACÍO - Verificar configuración en n8n",
            );
          }
          ...
          throw new Error(
            "RESPUESTA INVÁLIDA DEL WORKFLOW - Formato incorrecto",
          );
        }
        ...
        if (Array.isArray(result) && result.length > 0 && result[0].json) {
          ...
          return {
            text:
              ...
              "Respuesta del asistente de Grupo Lotificadora",
...
function listFiles(type, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al leer directorio: " + error.message });
  }
}
...
io.on("connection", (socket) => {
  // Capturar IP del cliente y timestamp de conexión
  const clientIP =
  ...
  const connectionTime = Date.now(); // Guardar timestamp de conexión

  // Guardar timestamp en el socket para usarlo en disconnect
  socket.connectionTime = connectionTime;

  console.log("🔌 Usuario conectado:");
  console.log(`   📱 Socket ID: ${socket.id}`);
  console.log(`   🌐 IP Cliente: ${clientIP}`);
  console.log(`   🕐 Timestamp: ${new Date(connectionTime).toISOString()}`);
  console.log(
    `   🌍 User-Agent: ${socket.handshake.headers["user-agent"] || "No disponible"}`,
  );
  ...
  socket.on("message", async (data) => {
    try {
      ...
      socket.emit("error", {
        ...
        error:
          "El servicio de atención no está disponible. Por favor intenta más tarde.",
    ...
    try {
      ...
      socket.emit("message", {
        ...
        content:
          response.text ||
          response.message ||
          "Archivo procesado por Grupo Lotificadora",
        attachments: response.attachments || [],
        timestamp: response.timestamp || new Date(),
      });
    } catch (error) {
      ...
      socket.emit("error", {
        ...
        error:
          "No se pudo conectar con el sistema de archivos de Grupo Lotificadora.",
  ...
  socket.on("disconnect", () => {
    ...
    const connectionTime = socket.connectionTime || disconnectTime;
    const duration = Math.max(0, (disconnectTime - connectionTime) / 1000);

    console.log("🔌 Usuario desconectado:");
    console.log(`   📱 Socket ID: ${socket.id}`);
    console.log(`   🌐 IP Cliente: ${clientIP}`);
    console.log(`   🕐 Timestamp: ${new Date(disconnectTime).toISOString()}`);
...
server.listen(PORT, () => {
  console.log(`\n🏢 ASISTENTE VIRTUAL GRUPO LOTIFICADORA v. 0.0.55-`);
  console.log(`================================================`);
